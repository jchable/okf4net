...
- **A green xunit run says nothing about code xunit cannot execute.** The suite runs on .NET, so it cannot execute the JavaScript in `src/OKF4net.Viewer/Assets/`. A test that greps a `.js` file for a marker string passes just as happily when the code behind that marker has been gutted — this is not hypothetical: two "raw HTML disabled" tests were green while the sanitizer had an exploitable hole. Any non-.NET code carrying a real guarantee needs its own executable guard (for the viewer: `tools/viewer-security-check/`, run in CI as the `viewer sanitizer (JS)` job), and a source-text assertion must say in its own comment that it is a smoke check, not proof.
  - **The same applies to the Python that `src/OKF4net.Attestation.Containers/` ships inside C# string constants** (`SqlClientComputationExecutor.Wrapper`, `ContainerAttester.Bootstrap`). Its executable guard is `tests/OKF4net.Tests/Attestation.Containers/ContainerIntegrationTests.cs`, which needs a real engine and is **excluded from CI by decision** (`[Trait("Category", "ContainerIntegration")]`, filtered out in both `ci.yml` and `release.yml`, mirroring `producers/`). So a green CI says nothing at all about that Python: the guarantee is one local command, `dotnet test tests/OKF4net.Tests --filter Category=ContainerIntegration`, with Docker running and `OKF_DEMO_PG_CONN` set — run it after any change to a wrapper, to the engine, or after merging anything into this area. Every one of those tests skips without Docker, and two of them also without `OKF_DEMO_PG_CONN`, so check the run reports **0 skipped** before believing it. And note what even a real run cannot reach: removing the wrapper's `stdout=subprocess.DEVNULL` leaves the suite green, because pip is quiet on `python:3.12-slim` — that redirect is structural insurance, and its test says so in its own docstring rather than claiming to prove it.
...
the sanctioned text, and under this host that comparison **cannot fail** —
