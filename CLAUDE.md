...
- **`src/OKF4net.Viewer/`** — static HTML site generation for a bundle, referencing only `OKF4net` (BCL otherwise; zero `PackageReference`). Backs the `okf render` verb. Three units: `SiteModel` (pure `Bundle` → display-model projection), `HtmlWriter` (the only I/O), `ViewerAssets` (embedded CSS/JS). Markdown is rendered **client-side** by a vendored copy of marked (MIT, v15.0.12, credited in `NOTICE`) — the generated page carries its raw markdown plus a link-rewiring table as an HTML-safe JSON payload, escaped by `HtmlSafeJson` so untrusted bundle content cannot break out of the `<script>` container. Raw HTML is neutralized by a **two-layer** defense in `viewer.js`, not by marked itself (it has no `sanitize` option any more): layer 1 overrides marked's `html` renderer hooks (both the main `Renderer` and its separate `TextRenderer`) to suppress raw-HTML passthrough; layer 2 — the control that actually holds — runs the parsed DOM through a sanitizer (element allowlist, a per-tag attribute allowlist that drops every `on*` handler, URL-scheme validation on `href`/`src`) before it ever touches the live page. Layer 2 exists because marked's `Renderer.image()` interpolates the `alt` attribute with no escaping at all — `![foo" onerror="alert(1)](x.png)` breaks out of the attribute with no raw-HTML token involved, a class of bug layer 1 cannot see, let alone stop. **Do not delete layer 2 as "the redundant sanitizer"** — it is the layer that actually closes the gap. This defense has no automated regression guard in CI: xunit runs on .NET and cannot execute JavaScript, so `tests/OKF4net.Tests/Viewer/ViewerAssetsTests.cs` only smoke-checks for source-text markers (allowlist names, rejected schemes), not real sanitizer behavior. It was verified by hand instead — a Node/jsdom harness run against the real vendored marked, plus a manual browser check — but a future marked bump could regress it silently; see the caveat in `ROADMAP.md`. No full-text search by design: a static site has no server to run `ConceptSearch`, and mirroring its weights in JS would fork the scorer — search lands with the planned `okf serve`.
