...
// The text layer lives in LiftedMarkdown.cs. Every transform applied to text this producer did not
// author is a pure string function whose correctness is argued against `LinkScanner`, not against
// anything in this file, and reading the two rule sets side by side is the whole point of the split.
// Imported statically so the call sites below read exactly as they did before the move: the seam is
// for whoever has to compare those rules, not a new spelling to learn here.
using static OkfProducer.Core.Generation.LiftedMarkdown;
