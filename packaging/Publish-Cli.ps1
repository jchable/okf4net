...
<#
.SYNOPSIS
    Native-AOT publishes the okf CLI for one RID and archives the result.
...
$binName = if ($isWindowsRid) { 'okf.exe' } else { 'okf' }

Write-Host "Publishing okf CLI for $Rid (version $Version)..."
dotnet publish $proj -c Release -r $Rid -p:Version=$Version
if ($LASTEXITCODE -ne 0) { throw "dotnet publish failed for $Rid" }

$bin = Join-Path $proj "bin/Release/net10.0/$Rid/publish/$binName"
if (-not (Test-Path $bin)) { throw "published $binName not found at $bin" }
...
$archive = Join-Path $OutDir "okf-$Version-$Rid.$archiveExt"
...
if ($isWindowsRid) {
    ...
    tar czf $archive -C $binDir $binName
