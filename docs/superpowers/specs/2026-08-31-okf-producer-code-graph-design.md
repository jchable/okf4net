...
>
> 1. **« aucun processus lancé, rien d'exécuté depuis l'arbre scanné » est faux**, et c'était un *élargissement* d'une phrase déjà correcte (la note du CLI dit « aucun `dotnet msbuild` lancé », qui est juste). `GenerateRun.Execute` lance `git symbolic-ref`, et `ConceptGenerator` lance `git show -s --format=%cI HEAD` puis `git rev-parse HEAD` — les trois via `GitRevision.RunGit`, `WorkingDirectory` positionné sur le dépôt scanné, à **chaque** run, `--no-msbuild` compris. L'exposition est bien moindre (aucun des trois ne déclenche de hook, de fsmonitor, ni de pager avec stdout redirigé), mais une divulgation de sécurité doit être exactement vraie. Formulation retenue partout : « aucun `dotnet msbuild` lancé, aucune logique MSBuild du dépôt évaluée ».
>
