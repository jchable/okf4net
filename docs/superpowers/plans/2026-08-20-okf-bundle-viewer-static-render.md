...
2. **§8.2 — a two-layer defense, not a `sanitize` option.** The spec said "marked doit être configuré pour ne pas laisser passer le HTML brut". Modern marked **removed** the `sanitize` option and passes raw HTML through by default; there is no such config flag. The plan initially assumed overriding the renderer's `html` hooks (to emit nothing for raw HTML tokens) would be an equivalent substitute. **That assumption was disproved during Task 7/11 execution**: marked's `Renderer.image()` interpolates the `alt` attribute with **no escaping call at all**, so a plain markdown image with no raw-HTML token anywhere in it -- `![foo" onerror="alert(1)](x.png)` -- breaks out of the attribute and fires an `onerror` handler on page load. A renderer-hook override can never see this class of bug, because the defect is in how marked *builds* the attribute string, not in a specific hookable call site.
   The mechanism that actually shipped is **two layers**: layer 1 is the `html` renderer-hook override described above (on both `Renderer` and the separate `TextRenderer`) -- kept as defense in depth, but it is **not** the control that holds. Layer 2, the control that actually closes the gap, is a DOM sanitizer in `viewer.js` that runs after `marked.parse()` and before the result touches the live page: an element allowlist, a per-tag attribute allowlist that drops every `on*` handler, and URL-scheme validation on `href`/`src`. See `src/OKF4net.Viewer/Assets/viewer.js` and the harness at `tools/viewer-security-check/` for the mechanism and its verification. Do not remove layer 2 as "the redundant sanitizer" -- it is the one that holds.
...
**Security (spec §8.2):** marked passes raw HTML through by default — the old `sanitize` option was removed and there is no config flag for this. What actually shipped in `viewer.js` is two layers, not one: layer 1 overrides the renderer's `html` hooks (defense in depth, **not** the control that holds); layer 2 -- the control that actually holds -- sanitizes the parsed DOM itself (element allowlist, per-tag attribute allowlist dropping every `on*` handler, URL-scheme validation on `href`/`src`) before it touches the live page, because marked's own `Renderer.image()` builds the `alt` attribute with no escaping at all and can be broken out of by a plain markdown image with no raw-HTML token involved. See the corrected deviation entry at the top of this plan and `tools/viewer-security-check/` for the harness that verifies this.
