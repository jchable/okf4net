...
```csharp
    ...
    public Lifecycle Lifecycle => Lifecycle.From(_map.Get("status")?.AsDisplayString(), _map.Get("stale_after")?.AsDisplayString());
...
- [ ] **Step 1: Update tests.** In `tests/OKF4net.Tests/OkfSpecTests.cs`, change the expected version to `"0.2"`. In `tests/OKF4net.Tests/CliTests.cs`, update any assertion expecting `OKF v0.1`/`spec v0.1`/`OKF v0.1 conformance` to `0.2` (search the file for `0.1`). Add:

```csharp
    [Fact]
    public void Version_flag_reports_spec_0_2()
    {
        var (code, stdout, _) = RunCli("-V");
        Assert.Equal(0, code);
        Assert.Contains("(OKF spec v0.2)", stdout);
    }
```
(Use the file's existing `RunCli` helper convention; match its return shape.)
...
```
Expected content (verify each line against the Task 9 rules; `appendix_a` has 4 concepts, all legacy-`timestamp`-stamped, `customers`/`users` missing recommended fields):
```
...
```
**Important:** the exact concept ordering depends on `bundle.Concepts` order — do not hand-force it; take whatever the tool emits, then read every line to confirm it matches a Task 9 rule (no spurious/missing diagnostics). The exit code must be `0`. Save the captured bytes as the new `tests/fixtures/golden/validate.out` **with LF endings** (the file is `-text` in `.gitattributes`; write `\n` only, no trailing CRLF). If your editor/shell rewrites line endings, use:
```bash
printf '%s' "$(cat /tmp/validate.new)" | sed 's/\r$//' > tests/fixtures/golden/validate.out
```
Confirm the diff is only the intended changes:
```bash
...
- [ ] **Step 3: Generate `validate-v02.out`.** Add a throwaway assertion or use the CLI. Since the v0.2 golden must be clock-deterministic and the CLI uses the system clock, generate this golden through a **test** that injects a `FixedClock`, not the CLI. In `GoldenParityTests.cs` add a test that validates `tests/fixtures/okf_v02` with `new FixedClock(new DateOnly(2026, 7, 27))`, renders the report exactly as the CLI does (diagnostics each on their own line, blank line, the `N concept(s); …` summary, then the conformance line), and compares to `tests/fixtures/golden/validate-v02.out`. First run it to FAIL, capture the actual rendered output, hand-verify each line against the Task 9 rules, and save it as the golden. The rendering helper:
```csharp
    private static string RenderValidate(Bundle bundle, IOkfClock clock)
    {
        ...
        sb.Append('\n')
          .Append($"{bundle.Count} concept(s); {report.ErrorCount} error(s), {report.WarningCount} warning(s), {report.Of(Severity.Info).Count()} info.\n")
          .Append(report.IsConformant ? $"✓ conformant with OKF v{OkfSpec.Version}\n" : $"✗ not conformant with OKF v{OkfSpec.Version}\n");
        return sb.ToString();
    }

    [Fact]
    public void V02_fixture_matches_golden()
    {
        var root = RepoRootRelative("tests/fixtures/okf_v02");   // reuse existing repo-root helper in this file
        var actual = RenderValidate(Bundle.Load(root), new FixedClock(new DateOnly(2026, 7, 27)));
        var golden = File.ReadAllText(GoldenPath("validate-v02.out"));  // reuse existing golden-path helper
        Assert.Equal(golden, actual);
    }
```
Adapt `RepoRootRelative`/`GoldenPath` to the helpers already present in `GoldenParityTests.cs` (this file already locates the repo root and reads goldens — reuse those, do not invent new ones). Save `validate-v02.exitcode` as `0` if the file's convention pairs an exitcode with each golden (mirror how `validate.exitcode` is used).
