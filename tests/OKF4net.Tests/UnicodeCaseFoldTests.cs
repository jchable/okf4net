...
/// <summary>
/// Tests for <see cref="UnicodeCaseFold"/>, verifying full Unicode default
/// case folding (including the Final_Sigma rule) and code-point (UTF-8 byte
/// order) comparison, as used for title sorting in IndexGenerator's
/// <c>BuildIndexText</c>.
/// </summary>
public class UnicodeCaseFoldTests
{
    [Fact]
    public void ToLowercase_maps_capital_i_with_dot_above_to_two_chars()
    {
        ...
        // ToLowerInvariant leaves U+0130 unchanged; full Unicode lowercasing
        // does not.
        Assert.Equal("i̇", UnicodeCaseFold.ToLowercase("İ"));
    }
    ...
    public void ToLowercase_applies_final_sigma_rule()
    {
        ...
        Assert.Equal("οδος", UnicodeCaseFold.ToLowercase("ΟΔΟΣ"));
        ...
        Assert.Equal("σος", UnicodeCaseFold.ToLowercase("ΣΟΣ"));
    }

    [Fact]
    public void ToLowercase_skips_ascii_case_ignorable_punctuation_when_locating_final_sigma()
    {
        // "ΟΣ.ι": the Σ is followed by '.' (case-ignorable) and then by the
        // cased ι -- so, skipping the ignorable dot, Σ IS followed by a cased
        // character and is therefore NOT final. It takes the default
        // mapping σ, not ς.
        Assert.Equal("οσ.ι", UnicodeCaseFold.ToLowercase("ΟΣ.ι"));
        ...
        // final and becomes ς.
        Assert.Equal("ος.", UnicodeCaseFold.ToLowercase("ΟΣ."));
    }

    [Fact]
    public void ToLowercase_leaves_pure_ascii_unchanged_from_ToLowerInvariant()
    {
        Assert.Equal("hello world", UnicodeCaseFold.ToLowercase("Hello World"));
        Assert.Equal("events_*", UnicodeCaseFold.ToLowercase("events_*"));
    }
    ...
    public void CompareCodePoints_orders_by_code_point_not_utf16_code_unit()
    {
        ...
        // (matching UTF-8 byte-wise ordering) says the opposite:
        // U+E000 < U+1F600.
        var bmpPrivateUse = "\uE000";
        var astral = "\U0001F600";

        Assert.True(UnicodeCaseFold.CompareCodePoints(bmpPrivateUse, astral) < 0);
    ...
    {
        Assert.True(UnicodeCaseFold.CompareCodePoints("apple", "banana") < 0);
        Assert.Equal(0, UnicodeCaseFold.CompareCodePoints("same", "same"));
        Assert.True(UnicodeCaseFold.CompareCodePoints("banana", "apple") > 0);
    }
}
